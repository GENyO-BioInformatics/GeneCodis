...
def getAnnotidsSample(entries=1):
    annotations = SQLquerier('annotation',columns=['annotation_source'],distinct=True)
    annotid_every_source = []
    for annotation in annotations.annotation_source:
        ...
        annotid_every_source.append(annotation_id.annotation_id[0])
    return(annotid_every_source)
...
def genes2GC4ids(genes,entrezTaxid=['9606']):
    filters = {'synonyms':{'regex':False, 'filtervalue':genes},
                     'id':{'regex':True,'filtervalue':entrezTaxid}}
    genes = SQLquerier('synonyms',columns=['id'],filter=filters)
    print(len(set(genes)))
    return(genes.id.tolist())
